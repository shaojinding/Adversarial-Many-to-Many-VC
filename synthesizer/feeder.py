...
class Feeder:
    ...
    def __init__(self, coordinator, metadata_filename, hparams):
        ...
        # Load metadata
        self._ppg_dir = os.path.join(os.path.dirname(metadata_filename), "ppgs")
        ...
        with tf.device("/cpu:0"):
            ...
            self._placeholders = [
                tf.placeholder(tf.float32, shape=(None, None, hparams.num_ppgs), name="inputs"),
            ...
            queue = tf.FIFOQueue(8, [tf.float32, tf.int32, tf.float32, tf.float32,
                                     tf.int32, tf.int32, tf.float32, tf.int32], name="input_queue")
            ...
            eval_queue = tf.FIFOQueue(1, [tf.float32, tf.int32, tf.float32, tf.float32,
                                          tf.int32, tf.int32, tf.float32, tf.int32], name="eval_queue")
    ...
    def _get_test_groups(self):
        ...
        # change input data to ppg
        input_data = np.load(os.path.join(self._ppg_dir, meta[2]))
        mel_target = np.load(os.path.join(self._mel_dir, meta[1]))
        # Create parallel sequences containing zeros to represent a non finished sequence
        token_target = np.asarray([0.] * (len(mel_target) - 1))
        embed_target = np.load(os.path.join(self._embed_dir, meta[3]))
        speaker_label = int(meta[7])
        return input_data, mel_target, token_target, embed_target, len(mel_target), speaker_label
    ...
    def _prepare_batch(self, batches, outputs_per_step):
        ...
        speaker_labels = np.asarray([x[5] for x in batches], dtype=np.int32)

        ####################################

        return inputs, input_lengths, mel_targets, token_targets, targets_lengths, \
               split_infos, embed_targets, speaker_labels

    def _prepare_inputs(self, inputs):
        max_len = max([len(x) for x in inputs])
        return np.stack([self._pad_target(x, max_len) for x in inputs]), max_len
    ...
    def _pad_input(self, x, length):
        print(np.pad(x, (0, length - x.shape[0]), mode="constant", constant_values=self._pad).shape)
